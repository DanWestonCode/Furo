...
HRESULT FluidShader::Initialize(ID3D11Device* _device, ID3D11DeviceContext* _deviceContext, TwBar* _bar, int _size)
{
	...
	TwStructMember fluidProps[] = {
		...
		{ "Dissipation", TW_TYPE_FLOAT, offsetof(SimulationVars, m_densityDissipation), "min=0.99 max=10 step=0.1" },
...
void FluidShader::ComputeBoundaryConditions(ID3D11DeviceContext* _deviceContext)
{
	...
	//Dispatch Shader
	_deviceContext->Dispatch((UINT)ceil(FluidSize / NUM_THREADS), (UINT)ceil(FluidSize / NUM_THREADS), (UINT)ceil(FluidSize / NUM_THREADS));
