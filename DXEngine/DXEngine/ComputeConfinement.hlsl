...
void ComputeConfinement(uint3 id : SV_DispatchThreadID)
{
	...
	float omegaT = length(_Vorticity[coordT]);
	float omegaB = length(_Vorticity[coordB]);
	float omegaR = length(_Vorticity[coordR]);
	float omegaL = length(_Vorticity[coordL]);
	float omegaU = length(_Vorticity[coordU]);
	float omegaD = length(_Vorticity[coordD]);
