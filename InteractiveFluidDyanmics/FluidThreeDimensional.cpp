...
void FluidThreeDimensional::Initialize(int _size)
{
	...
	int size = (_size + 2) * (_size + 2) * (_size + 2);
...
{
	m_fluidSolver2D->VelocityStep(m_gridSize, m_velocityX, m_prevVelX, m_velocityY, m_prevVelY, m_visc, dt);
	m_fluidSolver2D->DensityStep(m_gridSize, m_density, m_prevDensity, m_velocityX, m_velocityY, m_diffusion, dt);
}
